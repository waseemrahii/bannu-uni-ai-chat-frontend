...
const Message = ({ message }) => {
  ...
  if (isSystem) {
    return (
      <div className="flex justify-center my-3">
        <div className="bg-yellow-100 text-yellow-800 text-xs px-3 py-1.5 rounded-full shadow-sm border border-yellow-200">
          {message.content}
        </div>
      </div>
    )
  ...
  return (
    <motion.div
      className={`flex ${isAI ? "justify-start" : "justify-end"} mb-3 last:mb-0`}
      initial={{ opacity: 0, y: 6 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.15 }}
    >
      <div className={`flex ${isAI ? "flex-row" : "flex-row-reverse"} items-end gap-2 max-w-[85%]`}>
        ...
        {/* bubble */}
        <div
          className={`rounded-2xl px-3 sm:px-4 py-2 max-w-full text-sm relative shadow ${
            isAI
              ? "bg-white text-gray-800 border border-gray-200"
              : "bg-[#DCF8C6] text-gray-900"
          }`}
